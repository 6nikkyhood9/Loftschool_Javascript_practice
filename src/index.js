...
for(let i = 0; i < child.length; i++){
  if(child[i].nextSibling && child[i].nextSibling.tagName === "P"){
     }
     if(child[i].nodeType = 1){
      findAllPSiblings(child[i]);
     }
    result.push(child[i]); 
}
return result;
...
for(let child of where.childNodes){
  child.nodeType === 3 ? chid.remove() : deleteTextNodesRecursive(child);
}
}
/*
 Задание 7 *:
